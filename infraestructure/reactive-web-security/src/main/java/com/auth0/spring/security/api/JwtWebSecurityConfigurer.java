...
public class JwtWebSecurityConfigurer {
    ...
    public static JwtWebSecurityConfigurer forRS256(String audience, String issuer) {
        ...
        return new JwtWebSecurityConfigurer(audience,
                issuer,
                new JwtAuthenticationProvider(jwkProvider, issuer, audience));
    }
    ...
    public static JwtWebSecurityConfigurer forHS256WithBase64Secret(String audience, String issuer, String secret) {
        ...
        return new JwtWebSecurityConfigurer(audience,
                issuer,
                new JwtAuthenticationProvider(secretBytes, issuer, audience));
    }
    ...
    public ServerHttpSecurity configure(ServerHttpSecurity http) {
        ...
        exceptionHandling.setAuthenticationEntryPoint(new JwtAuthenticationEntryPoint());

        AuthenticationWebFilter authentication = new AuthenticationWebFilter(
                auth -> Mono.fromCallable(() -> provider.authenticate(auth)));
        authentication.setAuthenticationConverter(new JwtAuthenticationConverter());
